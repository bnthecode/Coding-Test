...
const HomePage = ({ users, setUsers, setUserPosts, selectedUsersPosts }) => {
  ...
  const [userPostDialogOpen, setDialogOpen] = React.useState(false);
  ...
  const sortUsers = ({ target: { value } }) => {
    const filteredList = users.filter((user) => user.name.includes(value));
  ...
  return (
    <div style={{ height: "100%" }}>
      ...
      {selectedUsersPosts.length ? (
        <Paper style={{ maxHeight: 'calc(100vh - 600px)', overflowY: 'scroll'}} elevation={16}>
